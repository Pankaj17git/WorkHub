...
/**
 * @swagger
 * /api/worker-connections/{id}/accept:
 *   post:
 *     tags: [Worker Connections]
 *     summary: Accept connection request
 *     description: Worker accepts an incoming connection request from another worker.
 *     security:
 *       - BearerAuth: []
 *     parameters:
 *       - in: path
...
export async function POST(
  ...
  try {
    ...
    if (!actor || !actor.worker) return apiResponse.forbidden("Only workers can accept connections");

    const { id } = await params;
    const conn = await WorkerConnectionService.acceptConnection(actor, BigInt(id));
